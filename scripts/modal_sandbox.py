...
def get_cached_image(cache_key: str, dockerfile_hash: str | None = None) -> modal.Image | None:
    """Try to get a cached image by cache key.

    Returns the cached image if found and valid, None otherwise.
    For dockerfile images, validates the hash matches if provided.
    """
    cache = load_cache()
    ...
    # For dockerfile type, validate the hash matches
    if dockerfile_hash is not None:
        if entry.dockerfile_hash != dockerfile_hash:
            return None

    try:
        return modal.Image.from_id(entry.image_id)
...
@click.option("--force-rebuild", is_flag=True, help="Force rebuild image, skip cache")
def create_default(force_rebuild: bool):
    ...
    if not force_rebuild:
        image = get_cached_image("default")
...
@create.command("rust")
@click.option("--force-rebuild", is_flag=True, help="Force rebuild image, skip cache")
def create_rust(force_rebuild: bool):
...
@click.option("--force-rebuild", is_flag=True, help="Force rebuild image, skip cache")
def create_dockerfile(dockerfile_path: str, force_rebuild: bool):
    ...
    cache_key = f"dockerfile:{dockerfile_path}"  # MUST match build-image key format!
    ...
    image = None
    if not force_rebuild:
        image = get_cached_image(cache_key, dockerfile_hash=dockerfile_hash)
        if image is not None:
            print("Using cached image", file=sys.stderr)
...
@click.option("--force-rebuild", is_flag=True, help="Force rebuild image, skip cache")
def create_computronium(gi_path: str, force_rebuild: bool):
    ...
    image = None
    if not force_rebuild:
        image = get_cached_image("computronium")
        if image is not None:
            print("Using cached image", file=sys.stderr)
...
@click.option("--force-rebuild", is_flag=True, help="Force rebuild image, skip cache")
def create_sculptor(gi_path: str | None, force_rebuild: bool):
    ...
    image = None
    if not force_rebuild:
        image = get_cached_image("sculptor")
        if image is not None:
            print("Using cached image", file=sys.stderr)
...
@create.command("mngr")
@click.option("--force-rebuild", is_flag=True, help="Force rebuild image, skip cache")
def create_mngr(force_rebuild: bool):
