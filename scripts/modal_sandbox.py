...
def _derive_image_from_base(
    ...
    if patch_file is not None:
        ...
        img = img.run_commands(
            f"cd {project_root} && git config user.email offload@offload && git config user.name offload && git add -A && git commit -q --allow-empty -m base && git apply --3way /tmp/offload.patch --allow-empty && rm /tmp/offload.patch"
        )
