...
**Base image (parent-commit caching)**: When `[checkpoint]` is absent (the default), the image built for the parent commit (HEAD~1) serves as the base image for subsequent runs. The current run applies a thin diff from the parent to HEAD on top of this base. Cached in git notes on the parent commit.
...
```
Base image (rebuilt when parent commit changes):
  Dockerfile base + source checkout + sandbox_init_cmd
  Built from the parent commit tree (exported via git, same as checkpoint).
  Cached in git notes on that commit.

Current image (rebuilt each run):
  Base image + thin diff of changes from parent to HEAD
```
...
- If the `[checkpoint]` section is absent, checkpoints do not exist. Offload uses the parent commit (HEAD~1) as the base: it exports the parent tree, builds from it, caches the result, and applies a thin diff to HEAD.
...
3. **If a checkpoint is found:**
   ...
   c. Rust generates a binary patch file locally (`git diff <checkpoint-sha> HEAD --binary`, plus untracked files detected via `git ls-files --others --exclude-standard`). The patch file includes both tracked changes and untracked files.
   d. If diff is empty and no untracked files: use checkpoint image directly (zero overhead).
   e. If non-empty: pass the patch file to the Python script, which applies it on top of the checkpoint image. The Python script receives only `--from-checkpoint` (image ID) and `--patch-file` (path to binary patch); all git logic stays in Rust.
...
The parent-commit path follows the same pipeline as the checkpoint path. The only difference is how the base commit is selected (HEAD~1 instead of nearest ancestor touching `build_inputs`).

1. Fetch notes from remote (best-effort).
2. Look up the parent commit (HEAD~1) in git notes for a cached base image (keyed by TOML config path).
3. If cached base image exists (cache hit): Rust generates a binary patch file locally (diff from parent to HEAD, plus untracked files), then passes it to the Python script via `--patch-file` to build a thin-diff image on top of the cached base. On failure (e.g. git not installed in image, apply failure), warn and fall back to full build.
4. If no cached base image (cache miss): export the parent commit tree via `git::export_tree()`, build the base image via `provider.prepare(context_dir=exported_tree)`, write the note on HEAD~1, push notes. Then apply thin diff (same as step 3).
...
Both Checkpoint and ParentBase follow the same unified procedure under `--no-cache`:
...
2. Resolve the base commit (same logic as normal path):
   ...
   - Without `[checkpoint]` config: use HEAD~1 as the base commit.
...
4. If no base commit is found (no checkpoint in window, or initial commit with no parent): full build (same as normal path).
...
A read-only diagnostic command that shows the current cache state for the working directory. This gives users visibility into the otherwise-opaque caching machinery. It works in both modes: with `[checkpoint]` config (checkpoint caching) and without it (parent-commit caching).
...
**Without `[checkpoint]` config:** Resolves the parent commit (HEAD~1) as the base. Shows the parent SHA. If there is no parent (initial commit), reports "no base commit" and next run mode as full build.
...
Example output (parent-commit mode, cache hit):
```
HEAD:               a1b2c3d4
Base commit:        e9f0a1b2 (parent, HEAD~1)
Cached image:       im-def456
Next run mode:      thin diff (1 files changed since parent)
```

Example output (parent-commit mode, no parent):
```
HEAD:               a1b2c3d4
...
| `offload run --no-cache` (without `[checkpoint]`) | Same as normal cache miss (resolve parent SHA, export parent tree, build base from `context_dir`, apply thin diff) -- but no note read/write. Produces the same image, just without persisting the result. |
| Empty diff since checkpoint | Use checkpoint image directly |
| Binary files in diff | `git diff --binary` + `git apply` handles them |
| New untracked files since checkpoint | Detected by Rust via `git ls-files --others --exclude-standard` and included in the binary patch file alongside tracked changes (not baked into the checkpoint image itself) |
| No `[checkpoint]` in config, first run (parent-commit caching) | Export parent tree, build base from `context_dir`, cache on parent commit (HEAD~1), then thin diff to HEAD |
| No `[checkpoint]` in config, cached (parent-commit caching) | Use cached parent image + thin diff to HEAD |
| No `[checkpoint]` in config, initial commit (parent-commit caching) | Full build, no caching |
...
- Existing `offload.toml` files without `[checkpoint]` continue to work. The new behavior is that the parent commit tree is exported and used to build a base image (cached in git notes on HEAD~1), and subsequent runs apply a thin diff from the parent to HEAD. This follows the same pipeline as checkpoint mode -- the only difference is that the base commit is HEAD~1 instead of the nearest checkpoint ancestor.
