...
mod tests {
    ...
    /// Helper to verify that all top-level keys in a TOML string are preserved
    /// after round-tripping through Config deserialization and re-serialization.
    /// If any keys are silently dropped, the generated config has keys that
    /// Config does not recognize (proving the bug).
    fn assert_toml_round_trips_without_loss(toml_str: &str) {
        let config: Config = toml::from_str(toml_str)
            .expect("TOML should deserialize into Config for round-trip check");
        let reserialized = toml::to_string(&config).expect("Config should serialize back to TOML");
        let original: toml::Value =
            toml::from_str(toml_str).expect("original TOML should parse as Value");
        let round_tripped: toml::Value =
            toml::from_str(&reserialized).expect("reserialized TOML should parse as Value");
        let original_keys: std::collections::BTreeSet<&str> = original
            .as_table()
            .expect("original should be a table")
            .keys()
            .map(|k| k.as_str())
            .collect();
        let round_tripped_keys: std::collections::BTreeSet<&str> = round_tripped
            .as_table()
            .expect("round-tripped should be a table")
            .keys()
            .map(|k| k.as_str())
            .collect();
        let dropped: Vec<&&str> = original_keys.difference(&round_tripped_keys).collect();
        assert!(
            dropped.is_empty(),
            "Top-level TOML keys were silently dropped during deserialization: {:?}. \
             This means init_config generated keys that Config does not recognize.",
            dropped
        );
    }

    /// Test that the TOML generated by `init_config` for pytest/local round-trips
    /// through Config without silently dropping keys. This currently fails because
    /// `test_id_format` is placed at the top level (not a Config field) and is
    /// silently ignored during deserialization.
    #[test]
    fn test_init_config_pytest_deserializes() {
        let toml_str = r#"# offload configuration file
...
"#;
        ...
        assert_toml_round_trips_without_loss(toml_str);
    }

    /// Test that the TOML generated by `init_config` for cargo/local round-trips
    /// through Config without silently dropping keys. This currently fails because
    /// `test_id_format` is placed at the top level (not a Config field) and is
    /// silently ignored during deserialization.
    #[test]
...
"#;
    ...
    /// Test that the TOML generated by `init_config` for default/local deserializes
    /// into `Config`. This currently fails for two reasons: (1) `test_id_format` is
    /// placed at the top level instead of inside `[framework]`, and (2) the default
    /// framework template is missing the required `test_id_format` field inside
    /// `[framework]`.
    #[test]
...
"#;
        ...
        assert!(
            result.is_ok(),
            "Expected default/local init config to deserialize successfully, but got error: {:?}",
            result.err()
        );
    }
}
