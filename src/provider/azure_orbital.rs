//! Azure Orbital provider — simplified configuration for running tests on Azure Orbital sandboxes.
...
use crate::config::{AzureOrbitalProviderConfig, SandboxConfig};
use crate::connector::{Connector, ShellConnector};

/// Provider that runs tests on Azure Orbital sandboxes with simplified configuration.
///
/// Unlike [`DefaultProvider`](super::default::DefaultProvider) which requires
/// explicit command strings, this provider generates the Azure Orbital commands
...
/// # Sandbox Lifecycle
///
/// Each sandbox is an Azure Orbital sandbox instance. The provider uses
/// `azure_orbital_sandbox.py` for all operations:
///
/// 1. **Create**: Provisions a new Azure Orbital sandbox from the prepared image
...
pub struct AzureOrbitalProvider {
    connector: Arc<ShellConnector>,
    config: AzureOrbitalProviderConfig,
    /// Set during `prepare()`.
...
impl AzureOrbitalProvider {
    /// Creates a new Azure Orbital provider from the given configuration.
    ///
    ...
    /// run the image build.
    pub fn from_config(config: AzureOrbitalProviderConfig) -> Self {
...
#[async_trait]
impl SandboxProvider for AzureOrbitalProvider {
    ...
    async fn prepare(
        ...
        let mut prepare_cmd = String::from("uv run @azure_orbital_sandbox.py prepare");
        ...
        debug!("Azure Orbital image prepared with ID: {}", image_id);
    ...
    async fn create_sandbox(&self, config: &SandboxConfig) -> ProviderResult<DefaultSandbox> {
        debug!("Creating Azure Orbital sandbox: {}", config.id);

        // Run create command to get sandbox_id
        let image_id = self.image_id.as_deref().ok_or_else(|| {
            ProviderError::ExecFailed(
                "Azure Orbital image ID not set; call prepare() before create_sandbox()"
                    .to_string(),
            )
        })?;
        let create_command = format!(
            "uv run @azure_orbital_sandbox.py create --cpu {} {}",
        ...
        if result.exit_code != 0 {
            return Err(ProviderError::ExecFailed(format!(
                "Azure Orbital create command failed: {}",
                result.stderr
            )));
        }
        ...
        if sandbox_id.is_empty() {
            return Err(ProviderError::ExecFailed(
                "Azure Orbital create command returned empty sandbox_id".to_string(),
            ));
        }

        debug!("Created Azure Orbital sandbox with ID: {}", sandbox_id);

        // Build command templates with sandbox_id placeholder for later substitution
        let exec_command =
            "uv run @azure_orbital_sandbox.py exec {sandbox_id} {command}".to_string();
        let destroy_command = "uv run @azure_orbital_sandbox.py destroy {sandbox_id}".to_string();
        let download_command =
            Some("uv run @azure_orbital_sandbox.py download {sandbox_id} {paths}".to_string());
...
mod tests {
    ...
    fn test_prepare_command_minimal() {
        ...
        let config = AzureOrbitalProviderConfig::default();
        ...
        assert_eq!(
            prepare_cmd,
            "uv run @azure_orbital_sandbox.py prepare --cached"
        );
    }

    #[test]
    fn test_prepare_command_with_dockerfile() {
        let config = AzureOrbitalProviderConfig {
        ...
        assert_eq!(
            prepare_cmd,
            "uv run @azure_orbital_sandbox.py prepare ./Dockerfile --cached"
        );
    }
    ...
    fn test_prepare_command_with_all_options() {
        ...
        assert_eq!(
            prepare_cmd,
            "uv run @azure_orbital_sandbox.py prepare ./Dockerfile.test --include-cwd --copy-dir=./src:/app/src --copy-dir=./tests:/app/tests"
        );
    }
}
