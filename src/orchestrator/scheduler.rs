...
#[derive(Clone)]
pub struct ScheduledBatch<'rec> {
    pub tests: Vec<TestInstance<'rec>>,
    estimated_load: Duration,
}

impl<'rec> ScheduledBatch<'rec> {
...
impl<'rec> Batch<'rec> {
    ...
    fn add(&mut self, test: TestInstance<'rec>, duration: Duration) {
...
pub struct Scheduler<'rec> {
    queue: Mutex<VecDeque<ScheduledBatch<'rec>>>,
    notify: tokio::sync::Notify,
    batch_count: usize,
    batch_sizes: Vec<usize>,
}

impl<'rec> Scheduler<'rec> {
    ...
    pub fn new(
        ...
        let individual_batches: Vec<ScheduledBatch<'rec>> = individual_tests
            .into_iter()
            .map(|t| {
                let load = durations
                    .get(t.id())
        ...
        let mut batches: Vec<Batch<'rec>> = (0..num_batches).map(|_| Batch::new()).collect();
    ...
    pub async fn pop(&self) -> Option<ScheduledBatch<'rec>> {
        loop {
    ...
    fn push(&self, batch: ScheduledBatch<'rec>) {
        match self.queue.lock() {
            Ok(mut q) => {
                q.push_back(batch);
    ...
    pub async fn register_running_batch<Fut, R>(&self, batch: &ScheduledBatch<'rec>, fut: Fut) -> R
    where
        Fut: Future<Output = R>,
    {
...
mod tests {
    ...
    async fn drain_batches<'rec>(scheduler: &Scheduler<'rec>) -> Vec<Vec<TestInstance<'rec>>> {
