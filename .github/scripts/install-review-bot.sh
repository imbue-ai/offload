    ...
    steps:
      ...
      - name: Run Claude Code
        ...
        run: |
          ...
          **Comment by @${{ github.event.comment.user.login }}** on file `${{ github.event.comment.path }}` line ${{ github.event.comment.line }}:
          ...
          - **APPLY**: Small to moderate code change suggestion (<40 net lines changed). You understand exactly what to change and are confident it's correct.
          - **FLAG**: Actionable suggestion but too large (≥40 lines), ambiguous, or you're not confident in the fix.
          ...
          PROMPT
          )" --dangerously-skip-permissions --max-turns 25 --verbose --output-format stream-json > /tmp/claude-transcript.jsonl 2>&1
      ...
      - name: Claude Code transcript
        ...
        run: |
          ...
          def fmt(text, indent=2, limit=2000):
              text = str(text)[:limit]
              return textwrap.indent(text, " " * indent)

          for line in open("/tmp/claude-transcript.jsonl"):
              line = line.strip()
              if not line:
                  continue
              try:
                  ...
                  for block in msg.get("message", msg).get("content", []):
                      if block.get("type") == "text":
                          ...
                          if name == "Bash":
                              ...
                              print(f"  📄 {inp.get('file_path', '')}")
                          elif name == "Edit":
                              print(f"  ✏️  {inp.get('file_path', '')}")
                              ...
                              print(f"  🔍 {inp.get('pattern', '')}")
                          elif name == "Grep":
                              print(f"  🔍 {inp.get('pattern', '')} in {inp.get('path', '.')}")
                          else:
