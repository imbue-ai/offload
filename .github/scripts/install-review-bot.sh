      ...
      - name: Run Claude Code
        ...
        run: |
          ...
          **Comment by @${{ github.event.comment.user.login }}** on file `${{ github.event.comment.path }}` line ${{ github.event.comment.line }}:
          ...
          PROMPT
          )" --dangerously-skip-permissions --max-turns 25 --verbose --output-format stream-json 2>&1 | tee /tmp/claude-transcript.jsonl

      - name: Show Claude Code transcript
        if: always()
        run: |
          python3 << 'PYEOF'
          import json
